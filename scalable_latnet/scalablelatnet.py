...
class ScalableLatnet:
    ...
    def logp_logistic(X, alpha, lambda_):
        ...
        mu = tf.math.log(alpha)
        return tf.subtract(tf.add(tf.subtract(tf.math.log(lambda_), tf.multiply(lambda_, X)), mu),
                           tf.multiply(tf.constant(2.0, dtype=ScalableLatnet.FLOAT),
                                       tf.math.log(tf.add(tf.constant(1.0, dtype=ScalableLatnet.FLOAT),
    ...
    def get_KL_logistic(X, posterior_alpha, prior_lambda_, posterior_lambda_, prior_alpha):
        ...
        logdiff = ScalableLatnet.logp_logistic(X, posterior_alpha, posterior_lambda_) - ScalableLatnet.logp_logistic(X,
                                                                                                                     prior_alpha,
                                                                                                                     prior_lambda_)
        logdiff = tf.linalg.set_diag(logdiff, tf.zeros((tf.shape(input=logdiff)[0], tf.shape(input=logdiff)[1]),
                                                       dtype=ScalableLatnet.FLOAT))  # set diagonal part to zero
        return tf.reduce_sum(input_tensor=tf.reduce_mean(input_tensor=logdiff, axis=[0]))
    ...
    def get_KL_normal(posterior_mu, posterior_sigma2, prior_mu, prior_sigma2):
        ...
        kl = tf.add(
            ...
            -_half + _half * tf.math.log(prior_sigma2) - _half * tf.math.log(posterior_sigma2))
        kl = tf.linalg.set_diag(tf.expand_dims(
            kl, -3), tf.zeros((1, tf.shape(input=kl)[0]), dtype=ScalableLatnet.FLOAT))
        return tf.reduce_sum(input_tensor=kl[0])
    ...
    def get_vairables(flags, D, Y):
        ...
        log_sigma2 = tf.Variable(tf.math.log(prior_sigma2), dtype=ScalableLatnet.FLOAT)

        mu_gamma = tf.Variable(prior_mu_gamma, dtype=ScalableLatnet.FLOAT)
        log_sigma2_gamma = tf.Variable(
            tf.math.log(prior_sigma2_gamma), dtype=ScalableLatnet.FLOAT)

        mu_omega = tf.Variable(prior_mu_omega, dtype=ScalableLatnet.FLOAT)
        log_sigma2_omega = tf.Variable(
            tf.math.log(prior_sigma2_omega), dtype=ScalableLatnet.FLOAT)

        log_alpha = tf.Variable(tf.math.log(prior_alpha), dtype=ScalableLatnet.FLOAT)

        log_sigma2_n = tf.Variable(
            tf.math.log(tf.constant(flags.get_flag('init_sigma2_n'), dtype=ScalableLatnet.FLOAT)), dtype=ScalableLatnet.FLOAT)

        log_lengthscale = tf.Variable(
            tf.math.log(tf.constant(flags.get_flag('init_lengthscale'), dtype=ScalableLatnet.FLOAT)),
            dtype=ScalableLatnet.FLOAT)

        log_variance = tf.Variable(
            tf.math.log(tf.constant(flags.get_flag('init_variance'), dtype=ScalableLatnet.FLOAT)), dtype=ScalableLatnet.FLOAT)
    ...
    def get_elbo(logger, flags, D, t, Y, sigma2_n, mu, sigma2, mu_gamma, sigma2_gamma, mu_omega, sigma2_omega, alpha,
        ...
        Nrf_by_D = ((flags.get_flag('n_rff'), D))
        # sampling for W
        z_W = tf.random.normal(S_by_N_by_N, dtype=ScalableLatnet.FLOAT)
        W = tf.multiply(z_W, tf.sqrt(sigma2)) + mu
        W = tf.linalg.set_diag(W, tf.zeros((S, N), dtype=ScalableLatnet.FLOAT))
        print(W)

        # Gamma
        z_G = tf.random.normal(S_by_N_by_2Nrf, dtype=ScalableLatnet.FLOAT)
        Gamma = tf.multiply(z_G, tf.sqrt(sigma2_gamma)) + mu_gamma

        # Omega
        if flags.get_flag('learn_Omega') == 'var-resampled':
            z_O = tf.random.normal(S_by_Nrf_by_D, dtype=ScalableLatnet.FLOAT)
            ...
            Omega = tf.random.normal(S_by_Nrf_by_D, dtype=ScalableLatnet.FLOAT)
        else:
            raise Exception

        # sampling for A
        u = tf.random.uniform(S_by_N_by_N, minval=0, maxval=1, dtype=ScalableLatnet.FLOAT)
        _A = tf.math.divide(
            tf.add(
                tf.math.log(alpha),
                tf.subtract(
                    tf.math.log(u + eps),
                    tf.math.log(tf.constant(1.0, dtype=ScalableLatnet.FLOAT) - u + eps)
                )
            ),
            posterior_lambda_)
        A = tf.sigmoid(_A)
        A = tf.linalg.set_diag(A, tf.zeros((S, N), dtype=ScalableLatnet.FLOAT))
    ...
    def batch_ll_new(logger, flags, t, Y, sigma2_n, A, W, Gamma, Omega, variance, N, D, N_rf, T, S):
        ...
        Fi_under = tf.reshape(
            tf.matmul(Omega_temp, tf.transpose(a=t)), [S, N_rf, T])
        ...
        max_eig_check = abs(tf.reduce_max(input_tensor=eig_values)) < 1
        min_eig_check = abs(tf.reduce_min(input_tensor=eig_values)) < 1
        ...
        if approximate_inverse == 'approx':
            ...
            IB_inverse = tf.linalg.inv(IB)
            exp_Y = tf.matmul(IB_inverse, Z)
        real_Y = tf.expand_dims(tf.transpose(a=Y), 0)
        norm = tf.norm(tensor=exp_Y - real_Y, ord=2, axis=1)
        norm_sum_by_T = tf.reduce_sum(input_tensor=norm, axis=1)
        norm_sum_by_T_avg_by_S = tf.reduce_mean(input_tensor=norm_sum_by_T)
        ...
        first = tf.multiply(
            _half, tf.multiply(_N, tf.multiply(_T, tf.math.log(tf.multiply(_two_pi, sigma2_n)))))
    ...
    def cholesky_decompose(M):
        ...
        vec_transp = (tf.transpose(a=eig_vectors, perm=[0, 2, 1]))
        M = tf.matmul(vect_diag, vec_transp)
        M_chol = tf.linalg.cholesky(M)
        return M_chol
    ...
    def optimize(flags, s, D, t, Y, logger, callback=None):
        ...
        tf.random.set_seed(flags.get_flag('seed'))

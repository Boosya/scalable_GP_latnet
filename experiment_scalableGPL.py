...
from scalable_latnet.expr_util import Logger as scalable_latnet_Logger
...
def functional_connectivity_sim(Y, folder_name, s):
	...
	scalable_latnet_Logger.init_logger(path, "run", folder_name)
	(T,D) = Y[0].shape
	...
	elbo_, sigma2_n_,  mu, sigma2_, mu_gamma, sigma2_gamma_,mu_omega, sigma2_omega_, alpha_, lengthscale, variance = \
		Latnet.optimize(s, D, N_rf, norm_t, Y_data , opt_targets.keys() ,n_total_iter, opt_targets, scalable_latnet_Logger.logger,init_sigma2_n ,init_sigma2_g, init_lenthscale,init_variance,init_p, lambda_prior,lambda_postetior,var_lr,hyp_lr, n_samples, approximate_kernel, fix_kernel = fix_kernel)
	end_time = time.time()
	...
	for l in scalable_latnet_Logger.logger.handlers:
		l.close()
...
if __name__ == '__main__':
	...
	# for sims in ['sim1','sim2','sim3']:
		...
		# for Ti in [50, 100, 200]:
			...
			for s in range(N_OBJECTS):
				configs.append({'sims': sims, 'Ti':Ti, 's':s})
	for i in range(len(configs)):
		...
		functional_connectivity_group(output_folder,input_file,method,s,Ti=Ti)
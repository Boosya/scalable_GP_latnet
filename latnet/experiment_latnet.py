...
from latnet.latnet.latnet import Latnet
# from latnet.expr_util import Logger as latnet_Logger
from latnet.latnet.expr_util import ExprUtil
import numpy as np
...
def functional_connectivity_sim(Y, folder_name, s, logger_name):
    ...
    hyp_lr = 0.001
    n_samples = 200
    elbo_, sigma2_n_, sigma2_g_, mu, sigma2_, alpha_, lengthscale, variance = Latnet.optimize(s, norm_t, Y_data,
                                                                                              opt_targets.keys(),
                                                                                              n_total_iter, opt_targets,
                                                                                              logger, init_sigma2_n,
                                                                                              init_sigma2_g,
                                                                                              init_lenthscale,
                                                                                              init_variance, init_p,
                                                                                              lambda_prior,
                                                                                              lambda_postetior, var_lr,
                                                                                              hyp_lr, n_samples)
    end_time = time.time()
...
if __name__ == '__main__':
    ...
    for sim in ['sim1']:
        for Ti in [50]:
            config = {'sims': sim, 'Ti': Ti, 's': args.s,
                      'output_folder': 'fmri/fmri_' + sim + '_scalableGPL/' + args.n,

...
class Latnet:
    ...
    @staticmethod
    def run_model(t, Y, init_sigma2_g, init_sigma2_n, init_lengthscle, init_variance, init_p, lambda_prior,
                  lambda_postetior, var_lr, hyp_lr, n_samples, seed=None, fixed_kernel=False):
        """
        ...
        log_sigma2_n, log_sigma2_g, mu, log_sigma2, log_alpha, log_lengthscale, log_variance = Latnet.get_vairables(t,
                                                                                                                    Y,
                                                                                                                    init_sigma2_g,
                                                                                                                    init_sigma2_n,
                                                                                                                    init_lengthscle,
        ...
        kl_normal, kl_logstic, ell = Latnet.get_elbo(t, Y, tf.exp(log_sigma2_n), tf.exp(log_sigma2_g), mu,
                                                     tf.exp(log_sigma2), tf.exp(log_alpha), tf.exp(log_lengthscale),
                                                     tf.exp(log_variance),
                                                     tf.constant(lambda_prior, dtype=Latnet.FLOAT), \
                                                     tf.constant(lambda_postetior, dtype=Latnet.FLOAT), init_p,
                                                     n_samples, seed=seed)
        ...
        if fixed_kernel:
            ...
            hyp_opt, _, hyp_nans = Latnet.get_opzimier(tf.negative(elbo),
                                                       [log_sigma2_n, log_sigma2_g, log_lengthscale, log_variance],
                                                       hyp_lr)

        return var_opt, hyp_opt, elbo, kl_normal, kl_logstic, ell, log_sigma2_n, log_sigma2_g, mu, log_sigma2, log_alpha, log_lengthscale, log_variance, var_nans, hyp_nans
    ...
    def kernel(X, variance, lengthscale, T):
        ...
        sq_dists = tf.add(tf.subtract(dist, tf.multiply(tf.cast(2., Latnet.FLOAT), tf.matmul(X, tf.transpose(X)))),
                          tf.transpose(dist))
        return tf.multiply(variance, tf.exp(tf.negative(tf.math.divide(tf.abs(sq_dists),
                                                                       tf.multiply(tf.cast(2.0, Latnet.FLOAT),
                                                                                   tf.square(
                                                                                       lengthscale)))))) + tf.constant(
            1e-5 * np.identity(T), dtype=Latnet.FLOAT)
    ...
    def logp_logistic(X, alpha, lambda_):
        ...
        return tf.subtract(tf.add(tf.subtract(tf.log(lambda_), tf.multiply(lambda_, X)), mu),
                           tf.multiply(tf.constant(2.0, dtype=Latnet.FLOAT), tf.log(
                               tf.add(tf.constant(1.0, dtype=Latnet.FLOAT),
    ...
    @staticmethod
    def get_elbo(t, Y, sigma2_n, sigma2_g, mu, sigma2, alpha, lengthscale, variance, prior_lambda_, posterior_lambda_,
                 init_p, n_samples, seed=None):
        """
        ...
        return Latnet.get_KL_normal(mu, sigma2, prior_mu, prior_sigma2), Latnet.get_KL_logistic(X, alpha, prior_lambda_,
                                                                                                posterior_lambda_,
                                                                                                prior_alpha), Latnet.batch_ll(
            Kt, t, Y, sigma2_n, sigma2_g, A, W, N, T, S)
    ...
    def batch_ll(Kt, t, Y, sigma2_n, sigma2_g, A, W, N, T, S):
        ...
        Eg = matmul(matmul(L, matmul(B, tf.transpose(B, [0, 2, 1]))), tf.transpose(L, [0, 2, 1]))
        ...
        Lt_Lf = tf.add(matmul(tf.transpose(tf.expand_dims(tf.tile(tf.expand_dims(lt, -1), [1, S]), -3)),
                              tf.expand_dims(ltilda_f, -2)), tf.constant(1.0, dtype=Latnet.FLOAT))

        logdet = tf.multiply(tf.cast(T, Latnet.FLOAT), tf.reduce_sum(tf.log(ln))) + tf.reduce_sum(tf.log(Lt_Lf))
    ...
    def converged(elbos):
        ...
        last_4_percent_changes = [abs(last_5_elbos[i] - last_5_elbos[i + 1]) / abs(last_5_elbos[i + 1]) for i in
                                  range(len(last_5_elbos) - 1)]
        if max(last_4_percent_changes) < 0.01:
            return True
        return False

    @staticmethod
    def optimize(s, t, Y, targets, total_iters, local_iters, logger, init_sigma2_g=0.01, init_sigma2_n=0.1,
                 init_lengthscle=0.1, init_variance=1.0, init_p=0.01, lambda_prior=1.0, lambda_postetior=0.1,
                 var_lr=0.01, hyp_lr=1e-4, n_samples=200, log_every=10, callback=None, seed=None, fix_kernel=False):
        with tf.Session() as sess:
            var_opt, hyp_opt, elbo, kl_normal, kl_logstic, ell, log_sigma2_n, log_sigma2_g, mu, log_sigma2, log_alpha, log_lengthscale, log_variance, var_nans, hyp_nans = Latnet.run_model(
                tf.cast(t, Latnet.FLOAT), Y, init_sigma2_g, init_sigma2_n, init_lengthscle, init_variance, init_p,
            ...
            logger.debug(
                "prior lambda={:.3f}; posterior lambda={:.3f}; variat. learning rate={:.3f}; hyper learning rate={:.3f}".format(
            ...
            while total_iters is None or _iter < total_iters:
                ...
                if 'var' in targets:
                    ...
                    for i in range(0, local_iters['var']):
                        try:
                            ...
                            if i % log_every == 0:
                                logger.debug(
                                    '  local {:d} iter elbo: {:.0f} (KL norm={:.0f}; KL logistic={:.0f}, ell={:.0f}), {:d} nan in grads (err= {:d}). '.format(
                                        i, output[0], output[1], output[2], output[3], output[5], output[5] != 0))
                ...
                if callback is not None:
                    sigma2_n_, sigma2_g_, mu_, sigma2_, alpha_, lengthscale_, variance_ = sess.run((
                                                                                                   tf.exp(log_sigma2_n),
                                                                                                   tf.exp(log_sigma2_g),
                                                                                                   mu,
                                                                                                   tf.exp(log_sigma2),
            ...
            elbo_, sigma2_n_, sigma2_g_, mu_, sigma2_, alpha_, lengthscale, variance = sess.run((elbo,
                                                                                                 tf.exp(log_sigma2_n),
                                                                                                 tf.exp(log_sigma2_g),
                                                                                                 mu, tf.exp(log_sigma2),
                                                                                                 tf.exp(log_alpha),
                                                                                                 tf.exp(
                                                                                                     log_lengthscale),
                                                                                                 tf.exp(log_variance)))

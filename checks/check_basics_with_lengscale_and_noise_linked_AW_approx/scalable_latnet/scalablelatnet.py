...
class ScalableLatnet:
    ...
    def __init__(self, flags, subject, dim, train_data, validation_data, test_data, true_conn, logger):
        ...
        # calculating ELBO
        self.elbo = self.ell - self.get_kl()
    ...
    def optimize(self):
        ...
        with open('signal_prediction.csv', 'a', newline='') as file:
            ...
            writer.writerow([])
            file.close()
        return self.mu_w_, self.sigma2_w_, self.alpa_
    ...
    def run_variables(self):
        ...
        self.elbo, self.pred_signals, self.real_signals, self.mu_w, tf.exp(self.log_sigma2_w), tf.exp(self.log_alpha)))
    ...
    def get_kl(self):
        kl_omega = get_dkl_normal(self.mu_o, tf.exp(self.log_sigma2_o), self.pr_mu_o, tf.exp(self.pr_log_sigma2_o))

...
def calculate_ell(n_mc, n_rf, n_nodes, dim, dtype, g, o, b, log_sigma2_n, log_variance, log_lengthscale, real_data,
    ...
    norm = tf.pow(d, 2)
    norm_sum_by_n = tf.reduce_sum(norm, axis=[1])
    norm_sum_by_t_avg_by_s = tf.reduce_mean(tf.reduce_sum(tf.sqrt(norm_sum_by_n), axis=[1]))
...
class ScalableLatnet:
    ...
    def __init__(self, flags, subject, dim, train_data, fold, test_data, true_conn, logger):
        ...
        # Calculate ELBO
        self.elbo = self.ell - self.kl_o - self.kl_w - self.kl_a - self.kl_g*float(flags.get_flag('kl_g_weight'))

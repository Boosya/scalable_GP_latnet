...
class ScalableLatnet:
    ...
    def __init__(self, flags, subject, dim, train_data, fold, test_data, true_conn, logger):
        ...
        self.ell, self.pred_signals, self.real_signals = calculate_ell(n_mc=self.n_mc, n_rf=self.n_rf,
        ...
        _, self.test_pred_signals, self.test_real_signals = calculate_ell(n_mc=self.n_mc, n_rf=self.n_rf,
                                                                          n_nodes=self.n_nodes, dim=self.dim,
                                                                          dtype=self.FLOAT, g=self.g, o=self.o,
                                                                          b=self.b, log_sigma2_n=self.log_sigma2_n,
                                                                          log_variance=self.log_variance,
                                                                          log_lengthscale=self.log_lengthscale,
        ...
        self.hyp_opt, _, self.hyp_nans = get_optimizer(tf.negative(self.elbo),
                                                       [self.log_variance, self.log_lengthscale], self.hlr)
    ...
    def log_optimization(self, gl_i, i):
        self.logger.debug("{gl_i:d} local {i:d} iter: elbo={elbo_:.0f} (ell {ell_:.0f}, "
